                        ...
                        <div class="card-header text-end">
                            ...
                            <div id="basic-1_filter" class="dataTables_filter">
                                ...
                                <form method="GET">
                                    ...
                                    <label>
                                        To:
                                        <input type="date" name="to_date" value="{{ request('to_date') }}"style="border: 1px solid #efefef; padding: 0 10px; margin-left: 10px; height: 37px; border-radius: 0;">
                                    </label>
                                    <input type="submit" value="Search" style="border: 1px solid #efefef; padding: 0 10px; margin-left: 10px; height: 37px; border-radius: 0;">
                                    <lable>
                                ...
                                <label>Search:
                                    <input type="text" id="searchInput" placeholder="Search..." style="border: 1px solid #efefef;padding: 0 10px;margin-left: 10px;height: 37px;border-radius: 0;">
                                </label>
                                <label>
                                    Collceter:
                                    <select id="collceter_id" style="border: 1px solid #efefef; padding: 0 10px; margin-left: 10px; height: 37px; border-radius: 0;">
                                        <option value="">-- Select an collceter --</option>
                                ...
                                <label>
                                    Rider:
                                    <select id="rider_captain" style="border: 1px solid #efefef; padding: 0 10px; margin-left: 10px; height: 37px; border-radius: 0;">
                                        <option value="">-- Select an Rider --</option>
                                ...
                                <label>
                                    <button onclick="assignCollecterRiderOrders()" type="submit" style="border: 1px solid #efefef; padding: 0 10px; margin-left: 10px; height: 37px; border-radius: 0;">Assign</button>
                                </label>
                            </div>
                        </div>

                        <!--<div class="card-header text-end">-->
                        <!--    <h3 class="text-start">Current Orders List</h3>-->
                        <!--    <div id="basic-1_filter" class="dataTables_filter"><label>Search:-->
                        <!--            <input type="text" id="searchInput" placeholder="Search..." style="border: 1px solid #efefef;padding: 0 10px;margin-left: 10px;height: 37px;border-radius: 0;">-->
                        <!--        </label>-->
                        <!--    </div>-->
                            ...
                            <table class="table table-hover"  id='orderTable'>
                                ...
                                @foreach ($orders as $order)
                                    <tr>
                                        ...
                                        {{-- <td>{{ $order->notes }}</td>--}}
                                        <td>
                                            {{--                                        {{ substr(optional($order)->deliveryAddress->address, 0, 20) }}--}}
                                            <img
                                                ...
                                                @php
                                                    ...
                                                    switch ($nextStatus) {
                                                        case 1:
                                                            ...
                                                            $iconClass = 'fas fa-check-circle'; // Example icon for status 1
                                                            break;
                                                        case 2:
                                                            $buttonClass = 'btn-info';
                                                            $iconClass = 'fas fas fa-spinner'; // Example icon for status 2
                                                            break;
                                                        case 3:
                                                            $buttonClass = 'btn-warning';
                                                            $iconClass = 'fas fa-truck text-white'; // Example icon for status 3
                                                            break;
                                                        case 7:
                                                            $buttonClass = 'btn-danger';
                                                            $iconClass = 'fas fa-exclamation-triangle text-white'; // Example icon for status 7
                                                            break;
                                                        case 8:
                                                            $buttonClass = 'btn-danger';
                                                            $iconClass = 'fas fa-times-circle text-white'; // Example icon for status 8
                                                            break;
                                                        default:
                                                            $buttonClass = 'btn-dark';
                                                            $iconClass = 'fas fa-question-circle text-white'; // Default icon
                                                    }
                                        ...
                                        <td>{{  optional($order)->orderCollector->name ?? 'Pending..' }}</td>
...
<script>
    ...
    function assignCollecterRiderOrders() {
        let selectedOrders = getSelectedOrders();

        if (selectedOrders.length > 0) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = "{{ route('assignuser') }}";
            ...
            // Rider ID
            const selectedRiderId = document.getElementById('rider_captain').value;
            ...
            const selectedCollecterId = document.getElementById('collceter_id').value;
            const collecterIdInput = document.createElement('input');
            collecterIdInput.type = 'hidden';
            collecterIdInput.name = 'collceter_id';
            collecterIdInput.value = selectedCollecterId;
            form.appendChild(collecterIdInput);
    ...
    //get selected order and show on map
    ...
    document.getElementById('searchInput').addEventListener('keyup', function () {
        ...
        rows.forEach(row => {
            const rowText = row.textContent.toLowerCase(); // Get all cell text combined
