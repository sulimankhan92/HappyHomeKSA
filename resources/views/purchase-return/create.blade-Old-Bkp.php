                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header pb-0">
                                <h3 class="card-title mb-0">Add Return Stock<span class="f-m-light mt-1 text-danger error-message"></span></h3>
                            ...
                            <div class="card-body">
                                <div class="col-md-12">
                                    <div class="card">
                                        ...
                                        <form class="form theme-form dark-inputs" method="POST" action="{{ route('purchase_invoice_find') }}" id="frm-stock-in" >
                                            @csrf
                                            @method('POST')
                                            <div class="row">
                                                <div class="col-sm-2">
                                                    <div class="form-group">
                                                        <label for="search-prod">Find/Scan Invoice #</label>
                                                        <select id="purchase_invoice_id" class="form-control purchase_invoice_id" name="purchase_invoice_id">
                                                            <option value="">Select Invoice #</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-sm-6">
                                                    <button type="button" class="btn btn-success stock-in-btn" id="stock-in-btn" style="margin-top: 25px;padding: 0.75rem 2rem;" > Find Invoice </button>
                                                </div>
                                            </div>
                                        </form>
                                            ...
                                            <tr class="row">
                                                <table class="table table-hover table-striped table-responsive invoice-table" id="tbl-invoice">
                                                    ...
                                                    <tbody id="tbody">
                                                        <form class="form theme-form dark-inputs" method="POST" action="{{ route('purchase_return') }}">
                                                            @csrf
                                                            @method('POST')

                                                            @if(isset($purchase))
                                                                <input type="hidden" name="invoice_no" value="{{ $purchase->invoice_no }}">
                                                                ...
                                                                @foreach($purchase->purchaseItems as $item)
                                                                    <tr>
                                                                        ...
                                                                        <td><input type="number" name="new_stocks[]" class="form-control" value="{{ $item->productVariant->quantity }}" readonly></td>
                                                                        ...
                                                                        <td>
                                                                            <input type="number" name="return_quantity[]" class="form-control" value="" data-original-quantity="{{ $item->productVariant->quantity }}"
                                                                            @if($item->productVariant->quantity==0) readonly @endif disabled>
                                                                        </td>
                                                                        <td>
                                                                            <div class=" p-0" style="margin-top: -2px;">
                                                                                <input id="checkbox1" type="checkbox" name="return_purchases[]" value="{{ $item->id }}" class="return_purchases-checkbox" data-return-total="{{ $item->item_total }}"
                                                                                       @if($item->productVariant->quantity==0) disabled @endif>
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                @endforeach
                                                            ...
                                                            <tr>
                                                                <td colspan="10">
                                                                    <button type="submit" class="btn btn-success" id="submit_returns"> Submit Returns </button>
                                                                </td>
                                                            </tr>
                                                            @endif
                                                        </form>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        returnQuantityInputs.forEach(input => {
            input.addEventListener('keyup', function () {
                ...
                const returnQuantity = parseInt(this.value); // Use `this.value` to get the current input value
        ...
        $('#purchase_invoice_id').select2({
            ...
            ajax: {
                ...
                processResults: function (data) {
                    return {
                        results: $.map(data.purchases, function (purchase) {
                            return {
                                id: purchase.id,
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const checkboxes = document.querySelectorAll('.return_purchases-checkbox');

        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', () => {
                const tr = checkbox.closest('tr'); // Get the parent TR element
                const input = tr.querySelector('input[name="return_quantity[]"]'); // Find the input element within the same TR

                if (checkbox.checked) {
                    // Enable the input and set a default value
                    input.disabled = false;
                    input.value = 1; // Or any default value you prefer
                } else {
                    // Disable the input and reset its value
                    input.disabled = true;
                    input.value = 0;
                }
            });
