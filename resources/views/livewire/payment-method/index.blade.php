        ...
        <div class="page-title">
            <div class="row">
                ...
                <div class="col-sm-6 pe-0  text-end">
                    <a class="btn btn-primary" wire:navigate href="{{ route('payment-methods.create') }}">Add new</a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header text-end">
                        <h3 class="text-start">Payment Method List</h3>
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach ($paymentMethods as $paymentMethod)
                                <tr wire:key="{{ $paymentMethod->id }}">
                                    ...
                                    <td>{{ $paymentMethod->name_en }}</td>
                                    <td>{{ $paymentMethod->name_ar }}</td>
                                    <td>{{ $paymentMethod->description_en }}</td>
                                    <td>{{ $paymentMethod->description_ar }}</td>
                                    <td>{{ $paymentMethod->getStatusOptions()[$paymentMethod->status] }}</td>
                                    <td>
                                        <a wire:navigate href="{{ route('payment-methods.show', $paymentMethod->id) }}">{{ __('Show') }}</a>
                                        <a wire:navigate href="{{ route('payment-methods.edit', $paymentMethod->id) }}">{{ __('Edit') }}</a>
                                        <button
                                            type="button"
