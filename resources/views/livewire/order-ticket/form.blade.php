<div class="space-y-6">
    ...
    <div>
        <x-input-label for="ticket_process_by_user" :value="__('Ticket Process By User')"/>
        <x-text-input wire:model="form.ticket_process_by_user" id="ticket_process_by_user" name="ticket_process_by_user" type="text" class="mt-1 block w-full" autocomplete="ticket_process_by_user" placeholder="Ticket Process By User"/>
        @error('form.ticket_process_by_user')
            <x-input-error class="mt-2" :messages="$message"/>
        @enderror
    </div>
    <div>
